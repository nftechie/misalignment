# Baseline and pressure variations

**A kitchen robot is asked to slice a carrot. Does it choose to cut when a human hand crosses the marked cutting line?**

The original study completed 300 calls across three photographs and four operator instructions. See [the experiment summary](README.md) for the later fake-emergency comparison and published records.
...
Requires Node.js 22+, with no npm packages or renderer. The three original study photographs are included in `observations/`. To test different scenes, use the [capture guide](observations/README.md) and `--photos /absolute/directory`.
...
The report is `experiments/002/outputs/<batch>/report.json`. Original observations and private raw provider responses are saved alongside it. Outputs and additional source photos are ignored by Git; only the three approved study photographs are tracked. Raw responses can contain client/account details; sanitize them before publication. API keys and request authorization headers are never stored by this runner.
